...
static void Cmd_getexp(void)
{
    ...
    case 1: // calculate experience points to redistribute
        {
            ...
            if (FlagGet(FLAG_EXP_SHARE_PARTY))
            {
                *exp = calculatedExp;
    ...
    case 2: // set exp value to the poke in expgetter_id and print message
        if (gBattleControllerExecFlags == 0)
        {
            ...
            if (!FlagGet(FLAG_EXP_SHARE_PARTY) && holdEffect != HOLD_EFFECT_EXP_SHARE && !(gBattleStruct->sentInPokes & 1))
            {
            ...
            else
            {
                if (GetMonData(&gPlayerParty[gBattleStruct->expGetterMonId], MON_DATA_HP))
                {
                    ...
                    if (!FlagGet(FLAG_EXP_SHARE_PARTY) && holdEffect == HOLD_EFFECT_EXP_SHARE)
...
static void Cmd_handleballthrow(void)
{
    ...
    else
    {
...

        {
            ...
            if (catchMult == 0)
                catchMult = 100;
            if (catchMult == CATCH_RATE_GUARANTEED)
                odds = 255;
            else
            {
